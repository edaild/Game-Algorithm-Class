   ...
   public void OnFindButton()
    {
        ...
        resultText.text =
            ...
            $"Quick Sort Compaarisons : {sortSteps} \n" +
            $"binary Sarch Total Comparisons: {binarySteps} \n" +
    ...
    private void QuickSort(List<Item> list, int left, int right)
    {
        if (left < right) return;
            ...
            QuickSort(list, left, pivotIndex - 1);               // 피벗 왼쪽 정렬
            QuickSort(list , pivotIndex + 1, right);              // 피벗 오른쪽 정렬
